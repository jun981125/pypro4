# Logistic Regression 클래스 사용 - 다항분류(활성화 함수 - softmax) - iris dataset
...
print(iris.keys()) # 멤버 확인
# print(iris.DESCR)
print(iris.feature_names) # ['sepal length (cm)', 'sepal width (cm)', 'petal length (cm)', 'petal width (cm)']
print(iris.target_names) # ['setosa' 'versicolor' 'virginica']
print(iris.data[:2])
print(np.corrcoef(iris.data[:,2], iris.data[:,3])) # 'petal length (cm)', 'petal width (cm)'의 상관계수 0.96286543 / numpy는 2개씩 밖에 못 봄
...
print()
'''
# scaling(크기를 고르게 만드는 장점이 있다.) - feature에 대해 표준화, 정규화 : 최적화(학습) 과정에서 안정성, 수련 속도를 향상 시키는 장점이 있다. 오버피팅 or 언더피팅 방지 기능이 있다.
print(x_train[:3])
sc = StandardScaler()
sc.fit(x_train)
sc.fit(x_test)
...
print('예측값 :', y_pred)
print('실제값 :', y_test)
print('총 계수 : %d, 오류 수 : %d'%(len(y_test), (y_test != y_pred).sum())) # 총 계수 : 45, 오류 수 : 1
...
del model # 실습이기에 시행한거. 현실에서는 del 안 함.

mymodel = joblib.load('cla4model.sav')
print('새로운 값으로 분류 예측 - "petal length (cm)", "petal width (cm)" - 스케일링해서 학습했다면 예측 데이터도 스케일링 해야함.')
print(x_test[:2])
new_data = np.array([[5.1, 2.4], [0.1, 0.1], [5.6, 5.6], [8.1, 0.5]])
new_pred = mymodel.predict(new_data) # softmax 함수가 제공한 결과에 대해 가장 큰 인덱스를 반환
print('예측 결과 :', new_pred) # 인덱스 값 표시
print('softmax 결과(날것) :\n', mymodel.predict_proba(new_data))

# 이항은 sigmoid(시그모이드) 다항은 softmax(소프트맥스)!! 이건 진리!!
